...
// Vendored from MetaMask/delegation-framework src/enforcers/TimestampEnforcer.sol (MIT AND
// Apache-2.0). Modifications: pragma widened from 0.8.23, import paths adapted. Contract name,
// terms layout, revert strings and logic are unchanged - running their enforcer unmodified on the
// Mapae manager is itself part of the compatibility proof.
 ...
 * timestamp restrictions on delegations.
...
contract TimestampEnforcer is CaveatEnforcer {
    ...
    /**
     * @notice Allows the delegator to specify the timestamp range within which the delegation
     * will be valid.
     * @param _terms - A bytes32 timestamp range where the first half of the word is the earliest
     * the delegation can be used and the last half of the word is the latest the delegation can be
     * used. The timestamp ranges are not inclusive.
    ...
    function beforeHook(
        ...
        address,
        address
    ) public view override onlyDefaultExecutionMode(_mode) {
