...
| **Graduated autonomy** | `VerifiedCodeEnforcer` is deployed and verified: Dojang's Verified Code attests an *off-chain human confirmation* (OTP-style, under a service domain), so high-value delegations can require a person in the loop per confirmation window while small ones run unattended — the historical mapae tiers, as caveats. The ladder is mined: one root with three rungs, and the shared day budget proven by a rung refused for what a *sibling* rung spent ([docs/DEMO.md](docs/DEMO.md)). What is still awaited is the first Verified Code issuer integration for the settling half; today only Upbit issues them, through its own channel, so only the refusal can be shown. |
